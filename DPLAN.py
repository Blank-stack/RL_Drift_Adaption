...
from rl.policy import EpsGreedyQPolicy, LinearAnnealedPolicy, SoftmaxPolicy
...
class DPLAN:
    ...
    def __init__(self, env: ADEnv, settings: dict):
        ...
        # optimizer = Adam(learning_rate=self.lr, epsilon=self.min_grad)
    ...
    def fit(self, env: ADEnv = None, weights_file=None):
        ...
        # Save model if the model_filename is given .
        # if model_file:
        #     self.save(model_file)

    def update(self, env: ADEnv = None, weights_file=None):
        if env:
            self.train_env = env
        else:
            self.train_env = self.env
        ...
        callbacks = DPLANCallbacks()
        self.agent.fit(env=self.train_env,
                       nb_steps=self.n_episodes * self.n_steps_episode,
                       action_repetition=1,
                       callbacks=[callbacks],
                       # nb_max_start_steps=10000,
                       # start_step_policy=SoftmaxPolicy(),
                       nb_max_episode_steps=self.n_steps_episode)
    ...
    def predict_original(self, X):
        # Return the orignal predicted.
        q_values = self.agent.model.predict(X[:, np.newaxis, :])

        return q_values

    def save(self, model_file):
        # Save the model's parameter and structure.
        self.agent.model.save(model_file, overwrite=True)
    #
    # def load_model(self, model_file):
    #     # load model from a stored file.
